...
class GetIssueUseCase @Inject constructor(private val repositories: RepositoriesItemRepository) {
    operator fun invoke(owner: String, repo: String): Flow<DataResult<MutableList<RepoIssueItem>>> = flow {
        try {
            emit(DataResult.Loading())
            val repoDetail = repositories.getRepositoryIssue(owner, repo)
