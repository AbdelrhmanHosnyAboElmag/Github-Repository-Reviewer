...
class GetRepositoriesUseCase @Inject constructor(private val repositories: RepositoriesItemRepository) {
    operator fun invoke(): Flow<DataResult<MutableList<RepositoriesItem>>> = flow {
        try {
            emit(DataResult.Loading())
            val repo = repositories.getRepositories()
